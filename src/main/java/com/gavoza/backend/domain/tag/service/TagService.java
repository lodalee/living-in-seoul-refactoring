...
public class TagService {
    ...
    public List<hashtagPostResponseDto> hashtagPostResponseDtos(int limit, String locationTagName) {
        List<hashtagPostResponseDto> hashtagPostResponseDtos = new ArrayList<>();

        List<Post> postList = postRepository.findAllByHashtagContaining(locationTagName);
        ...
        for (Post checkLocationTagName : postList) {
            String[] checkLocationTagNames = checkLocationTagName.getHashtag().split("#");

            //이제 확인해
            for (int i = 0; i < checkLocationTagNames.length; i++) {
                if (locationTagName.equals(checkLocationTagNames[i])) {
                    hashtagPostResponseDtos.add(new hashtagPostResponseDto(checkLocationTagName, locationTagName));
                    break;
                }
            }
