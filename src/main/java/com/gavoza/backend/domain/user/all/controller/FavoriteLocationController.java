...
public class FavoriteLocationController {
    ...
    @PostMapping("/add")
    public ResponseEntity<MessageResponseDto> addFavoriteLocation(@RequestBody FavoriteLocationRequestDto requestDto,
                                                                  HttpServletRequest request) {
        String email = jwtUtil.getEmailFromAuthHeader(request);
        favoriteLocationService.addFavoriteLocation(email, requestDto.getGu(), requestDto.getDong());
        return ResponseEntity.ok(new MessageResponseDto("찜한 위치가 추가되었습니다."));
    }

    @DeleteMapping("/delete/{locationId}")
    public ResponseEntity<MessageResponseDto> removeFavoriteLocation(@PathVariable Long locationId) {
        favoriteLocationService.removeFavoriteLocation(locationId);
